...
object MyModule {
  ...
  def fib(n: Int): Int = {
    require(n >= 0, "argument must be equal or greater than 0")

    @annotation.tailrec
    def go(count: Int, acc: Int, prev: Int): Int =
      if (count == n) acc
...
object PolymorphicFunctions {
   ...
   */
  def main(args: Array[String]): Unit = {
    val arr1 = Array(9, 2, 5, 11, 15)
    val el = 7
    println("element %d is present in array [%s]: %b".format(el, arr1.mkString(", "),
      binarySearch(arr1, el, (x: Int, y: Int) => x > y)))
    println("array [%s] is sorted: %b".format(arr1.mkString(", "),
      isSorted(arr1, (x: Int, y: Int) => x > y)))

    println("partial function result: %s".format(partial1(5, (x: Int, y: Int) => x + y)(20)))
    println("carrying function result: %s".format(curry((x: Int, y: Int) => x + y)(5)(20)))
    println("uncarrying function result: %s".format(uncurry[Int, Int, Int](x => y => x + y)(5, 20)))
    println("composition of 2 functions result: %s".format(
        compose[Int, Int, Int](y => y + 5, x => x*2)(20)))
  }
