...
object List { // `List` companion object. Contains functions for creating and working with lists.
  ...
  def init[A](l: List[A]): List[A] = l match {
    ...
    case Cons(h, Nil) => Nil // last element
  ...
  def map[A, B](l: List[A])(f: A => B): List[B] =
    reverse(foldLeft(l, Nil: List[B])((acc, h) => Cons(f(h), acc)))

  def filter[A](l: List[A])(f: A => Boolean): List[A] =
    reverse(foldLeft(l, Nil: List[A])((acc, h) => if (f(h)) acc else Cons(h, acc)))
  ...
  def startsWith[A](l: List[A], prefix: List[A]): Boolean = (l, prefix) match {
    case (_, Nil) => true
    case (Cons(lh, lt), Cons(ph, pt)) => if (lh != ph) false else startsWith(lt, pt)
    case _ => false
  }
